...
module.exports = async (req, res, next) => {
  ...
  try {
    ...
    const isRoleParticipant = userRole.role_participant;

    if (!isRoleParticipant) {
      return res.status(403).json({ error: "Not Authorized as Participant" });
    }

    next();
  } catch (error) {
    console.error("PARTICIPANT Authorization middleware error", error);
    return res.status(403).json({ error: "Not Authorized" });
  }
};
