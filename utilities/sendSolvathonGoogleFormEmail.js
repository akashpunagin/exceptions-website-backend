...
const sendPaymentVerificationEmail = async (name, email) => {
  ...
  try {
    const temp = await transport.sendMail({
      ...
      to: email,
      subject: "Exceptions 2023 - Google form for Solvathon Event",
      html: `
      ...
      >
        ...
        >
          ...
          >
            Hey, ${name}. Please fill the google form below regarding the details of Solvathon event
          </h3>
          <a
            href=${googleFormLink}
            style="
    ...
    console.error(
      "ERROR while sending google form solvathon event email",
      error
    );
    return false;
  }
